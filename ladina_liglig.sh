...
for dir in */; do
    ...
    lig1="$(basename "${mol_files[0]}" .mol2).acpype/$(basename "${mol_files[0]}" .mol2)_NEW.pdb"
    lig2="$(basename "${mol_files[1]}" .mol2).acpype/$(basename "${mol_files[1]}" .mol2)_NEW.pdb"

    lig1_id=$(awk 'NR==4{print $4}' "$lig1")
    lig2_id=$(awk 'NR==4{print $4}' "$lig2")

    file1="$(basename "${mol_files[0]}" .mol2).acpype/$(basename "${mol_files[0]}" .mol2)_GMX.itp"
    file2="$(basename "${mol_files[1]}" .mol2).acpype/$(basename "${mol_files[1]}" .mol2)_GMX.itp"
	...
	grep -h ATOM "$lig1" "$lig2" >| complex.pdb;
